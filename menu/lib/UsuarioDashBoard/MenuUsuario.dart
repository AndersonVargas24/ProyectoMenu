...
class MenuUsuario extends StatefulWidget {
  ...
  const MenuUsuario({super.key, this.comandaParaEditar});
...
class _MenuUsuarioState extends State<MenuUsuario> {
  ...
  void _anadirItem(Map<String, dynamic> menuItem) async {
    setState(() {
      final index =
          _itemsComanda.indexWhere((item) => item['nombre'] == menuItem['nombre']);
      if (index != -1) {
  ...
  void _eliminarItem(int index) async {
    final confirm = await showDialog(
      ...
      builder: (_) => AlertDialog(
        ...
        content:
            const Text('¿Estás seguro de eliminar este item de la comanda?'),
        actions: [
          TextButton(
  ...
  double _calcularTotalComanda() =>
      _itemsComanda.fold(0, (total, item) => total + item['precio'] * item['cantidad']);
    ...
    try {
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: const Text('Error al seleccionar la hora'),
            backgroundColor: Colors.red[400],
          ),
        );
      }
  ...
  Future<void> _guardarComanda() async {
    ...
    try {
      ...
      final comandaData = {
        ...
        'fecha_creacion':
            _esModoEdicion ? widget.comandaParaEditar!['fecha_creacion'] : DateTime.now(),
              ...
              panel: Container(
                ...
                child: Column(
                  children: [
                    ...
                    // Nuevo Dropdown para método de pago
                    Padding(
                      ...
                      child: DropdownButtonFormField<String>(
                        ...
                        decoration: InputDecoration(
                          ...
                          filled: true,
                          fillColor: Colors.blue[50],
                          prefixIcon: Icon(Icons.payment, color: Colors.blue[400]),
                        ),
                        items: <String>['Efectivo', 'Tarjeta de Crédito', 'Tarjeta Débito', 'Nequi', 'DaviPlata']
                    ...
                    Padding(
                      ...
                      child: TextField(
                        controller: _comentarioController, // Usar el controlador aquí
                        decoration: InputDecoration(
